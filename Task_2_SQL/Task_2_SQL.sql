...
/*Query 1*/
drop table NoOfOrders;
Create table NoOfOrders as 
SELECT c.customerName as a ,count(o.orderNumber) as b FROM customers AS c inner join orders AS o where c.customerNumber=o.customerNumber 
group by c.customerNumber;
Select a,max(sub.b) from (SELECT c.customerName as a ,count(o.orderNumber) as b FROM customers AS c inner join orders AS o where c.customerNumber=o.customerNumber group by o.customerNumber) as sub;
select sub.a,max(sub.b) from (SELECT c.customerName as a ,count(o.orderNumber) as b FROM (customers AS c inner join orders AS o on c.customerNumber=o.customerNumber) group by c.customerNumber) as sub;
Select a , max(b)  from NoOfOrders ;
/*correct query*/
select a ,b from nooforders where b = (select max(b) from nooforders);
...
select e.firstName, e.lastName , sum(p.amount) from((employees as e inner join customers as c on e.employeeNumber=c.salesRepEmployeeNumber )inner join payments as p on c.customerNumber=p.customerNumber )group by e.employeeNumber;
/*Query 4*/

select p.productName , o.orderdate from ((products as p inner join orderdetails as od on p.productCode=od.productCode)inner join  orders as o on od.orderNumber=o.orderNumber ) where o.orderDate BETWEEN '2005-01-01' AND '2005-01-31';
...
INSERT INTO employeedetails (bankAccount, address, phoneNumber, personalEmail)
VALUES ('AL02394029', 'Tirane', '0685035045', 'flaviopernoj2024@gmail.com');
